# How to provision a new environment for graphql-app
...
* Enter webroot path: `/home/deployer/apps/graphql_app/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/graphql_app.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/graphql_app.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/graphql_app/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
23 3,15 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/graphql_app/shared/public/
```
